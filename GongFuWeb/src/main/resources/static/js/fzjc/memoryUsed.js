...
function displayMemoryUsed(url, id, reloadFrequency,params) {
	...
	function generate(baseData, id) {
		...
		var margin = {
			...
			bottom : 25,
			left : 50
		}, width = 383 - margin.left - margin.right, height = 200 - margin.top - margin.bottom;
		...
		var yAxis = d3.svg.axis()
			.scale(y)
			.tickSize(-width)
			.tickValues([0.2, 0.4, 0.6, 0.8, 0.9, 1.0])
		...
		var onG = svg.append("g")
				.on("mousemove",function(d) {
					...
					// 	if (startTime +"" == newData[0].values[ii].time +"") { // 对比数据获取当前时间对应的y轴值，以便确定生成标记的位置
					...
					// // if( display == 'none' ){
					// $("#tooltip-p").css({"display" : "", "left" : "" + (xy[0] - 50) + "px", "top" : "" + xy[1] + "px"});
					// $("#tooltip-p").find("span").html(format(startTime));
					// d3.selectAll(".tagI").remove(); // 删除前一次移动生成的点
					// $("#tooltip-p").find("div").empty();
					// if( used == undefined ) used = 0;
					// if( free == undefined ) free = 0;
					// $("#tooltip-p").find("div").append("</br><i class='table_legend' style='background-color:rgba(19, 159, 222, 1);'></i><span> used："+ used +"%</span>" +
					// 									"</br><i class='table_legend' style='background-color:rgba(231, 126, 0, 1);'></i><span> free："+ free +"%</span>");
	...
	function redraw(data, x, y, xAxis, svg, area, onG) {
		...
		svg.select("#memoryUsed1-x-axis").transition().duration(200).ease(
