    ...
    public Map<String,Object> getData_new(@RequestBody EsQueryBean esQueryBean){
        ...
        try {
            if(esQueryBean == null){
                ...
                list = boolTermQueryI.query_new(esQueryBean.getIndices(), esQueryBean.getTypes(), esQueryBean.getParameters());
                outMap.put(KEY_RESULT,VAL_SUCCESS);
                outMap.put(KEY_RESULTDATA,list);
    ...
    public Map<String,Object> getData(@RequestBody EsQueryBean esQueryBean){
        ...
        try {
            if(esQueryBean == null){
                ...
                list = boolTermQueryI.query(esQueryBean.getIndices(), esQueryBean.getTypes(), esQueryBean.getParameters());
                outMap.put(KEY_RESULT,VAL_SUCCESS);
                outMap.put(KEY_RESULTDATA,list);
                outMap.put(KEY_MESSAGE,"获取数据成功");
//                System.out.println(JSON.toJSONString(esQueryBean));
    ...
    public Map<String,Object> getData_resultId(@RequestBody EsQueryBean esQueryBean){
        ...
        try {
            if(esQueryBean == null){
                ...
                resultMap = boolTermQueryI.query_resultId(esQueryBean.getIndices(), esQueryBean.getTypes(), esQueryBean.getParameters());
                outMap.put(KEY_RESULT,VAL_SUCCESS);
                outMap.put(KEY_RESULTDATA,resultMap);
                outMap.put(KEY_MESSAGE,"获取数据成功");
            }
	...
	public Map<String,Object> getData_temp(@RequestBody EsQueryBean esQueryBean){
		...
		try {
			if(esQueryBean == null || esQueryBean.getParameters() == null){
				...
				list = esService.find_2(esQueryBean.getIndices(), esQueryBean.getTypes(), esQueryBean.getTemplataName(), esQueryBean.getParameters());
				outMap.put(KEY_RESULT,VAL_SUCCESS);
				outMap.put(KEY_RESULTDATA,list);
				outMap.put(KEY_MESSAGE,"获取数据成功");
			}
	...
	public Map<String,Object> getAlertData(@RequestBody EsQueryBean esQueryBean){
		...
		try {
			if(esQueryBean == null){
				...
				list = esConfigService.getConfigAlert(esQueryBean.getIndices(), esQueryBean.getTypes(), esQueryBean.getParameters());
				outMap.put(KEY_RESULT,VAL_SUCCESS);
				outMap.put(KEY_RESULTDATA,list);
				outMap.put(KEY_MESSAGE,"获取数据成功");
//                System.out.println(JSON.toJSONString(esQueryBean));
